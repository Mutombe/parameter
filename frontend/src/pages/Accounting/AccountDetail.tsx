...
export default function AccountDetail() {
  ...
  const { data: statement, isLoading: loadingLedger } = useQuery({
    queryKey: ['account-statement', accountId],
    queryFn: () => glApi.accountStatement({ account: accountId }).then(r => r.data),
    enabled: !!accountId,
  ...
  return (
    <div className="space-y-6">
      <nav className="flex items-center gap-2 text-sm text-gray-500">
        ...
        <span>/</span>
        <button onClick={() => navigate('/dashboard/global-accounts')} className="hover:text-gray-900">Assets & Liabilities</button>
        <span>/</span>
        <span className="text-gray-900 font-medium">{account?.name || '…'}</span>
      </nav>

      <button onClick={() => navigate('/dashboard/global-accounts')} className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900">
        <ArrowLeft className="w-4 h-4" /> Back to Assets & Liabilities
      </button>
