...
export default function BalanceSheetMovements() {
  ...
  const [showDetail, setShowDetail] = useState<BSMovement | null>(null)
  ...
  const { data: movementsData, isLoading, error } = useQuery({
    queryKey: ['bs-movements', statusFilter, debouncedSearch, currentPage],
    queryFn: async () => {
      const params: Record<string, string | number> = { page: currentPage, page_size: PAGE_SIZE }
      if (statusFilter) params.status = statusFilter
      if (debouncedSearch) params.search = debouncedSearch
      const response = await bsMovementApi.list(params)
      return response.data
    },
    placeholderData: keepPreviousData,
  })

  const movements: BSMovement[] = movementsData?.results || movementsData || []
  ...
  const postMutation = useMutation({
    mutationFn: (id: number) => bsMovementApi.postToLedger(id),
    onSuccess: () => {
      toast.success('Movement posted to ledger')
      queryClient.invalidateQueries({ queryKey: ['bs-movements'] })
    },
    onError: () => toast.error('Failed to post movement'),
  })

  // Delete mutation
  const deleteMutation = useMutation({
    mutationFn: (id: number) => bsMovementApi.delete(id),
    onSuccess: () => {
      showToast.success('Movement deleted')
      queryClient.invalidateQueries({ queryKey: ['bs-movements'] })
    },
    onError: (err) => showToast.error(parseApiError(err)),
  })
  ...
  const stats = {
    ...
    posted: movements.filter(m => m.status === 'posted').length,
  }

  const selectableItems = movements.filter((m: any) => !m._isOptimistic)
  const pageIds = selectableItems.map(m => m.id)

  const handleBulkExport = () => {
    const selected = selectableItems.filter(m => selection.isSelected(m.id))
    exportTableData(selected, [
      { key: 'movement_number', header: 'Number' },
      { key: 'date', header: 'Date' },
      { key: 'debit_account_name', header: 'Debit Account' },
      { key: 'credit_account_name', header: 'Credit Account' },
      { key: 'category', header: 'Category' },
      { key: 'landlord_name', header: 'Landlord' },
      { key: 'amount', header: 'Amount' },
      { key: 'status', header: 'Status' },
    ], 'bs_movements_export')
    showToast.success(`Exported ${selected.length} movements`)
  }
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  ...
                  <th className="px-4 py-3 text-left font-medium text-gray-500">Debit Account</th>
                  <th className="px-4 py-3 text-left font-medium text-gray-500">Credit Account</th>
                    ...
                    return (
                      <motion.tr
                        ...
                        className={cn(
                          ...
                          selection.isSelected(movement.id) && 'bg-primary-50/30'
                        )}
                      >
                        <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                          <SelectionCheckbox
                            checked={selection.isSelected(movement.id)}
                            onChange={() => selection.toggle(movement.id)}
                          />
                        </td>
                        <td className="px-4 py-3 font-medium text-gray-900">{movement.movement_number}</td>
                        <td className="px-4 py-3 text-gray-600">{formatDate(movement.date)}</td>
                        <td className="px-4 py-3 text-gray-600">
                          <span className="text-xs text-gray-400 mr-1">{movement.debit_account_code}</span>
                          {movement.debit_account_name}
                        </td>
                        <td className="px-4 py-3 text-gray-600">
                          <span className="text-xs text-gray-400 mr-1">{movement.credit_account_code}</span>
                          {movement.credit_account_name}
                        </td>
                        <td className="px-4 py-3 text-gray-600 capitalize">{movement.category}</td>
                        <td className="px-4 py-3 text-gray-600">{movement.landlord_name}</td>
                        ...
                        <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-end gap-1">
                            {movement.status === 'draft' && (
                              <>
                                <button
                                  onClick={() => postMutation.mutate(movement.id)}
      ...
      <Modal
        ...
        title={showDetail ? `Movement ${showDetail.movement_number}` : ''}
        size="lg"
      >
        {showDetail && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">{showDetail.debit_account_code} - {showDetail.debit_account_name}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Credit Account</p>
                <p className="font-medium">{showDetail.credit_account_code} - {showDetail.credit_account_name}</p>
              </div>
              <div>
