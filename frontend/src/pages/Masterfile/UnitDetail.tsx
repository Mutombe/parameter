...
import { Button, TableFilter, Modal, Tabs, TabsList, TabsTrigger, TabsContent, DatePicker, DetailCurrencyGate } from '../../components/ui'
...
export default function UnitDetail() {
  ...
  const activeTenantId = activeLease?.tenant_id || activeLease?.tenant || unit?.current_tenant_id || unit?.current_tenant
  ...
  const { data: subAccountStatement, isLoading: loadingSubStatement } = useQuery({
    queryKey: ['unit-tenant-sub-statement', tenantSubAccount?.id, subAccountDateRange, subAccountStatementView, detailCurrency],
      ...
      {activeTenantId ? (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          {loadingSubAccounts ? (
            ...
            <div className="p-12 text-center text-sm text-gray-500">Select a currency above to view this account&rsquo;s transactions.</div>
          ) : (
            <>
              {subAccountStatement && (
