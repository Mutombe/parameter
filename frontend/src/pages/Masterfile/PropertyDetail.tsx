...
export default function PropertyDetail() {
  ...
  // Sub-account statement (individual). `isFetching` (not just isLoading)
  // drives the skeleton so toggling Consolidated ↔ Audit shows a loading
  // state even though `keepPreviousData` keeps the previous payload visible.
        ...
        <div className="p-6">
          {loadingSubAccounts ? (
            <div className="flex flex-wrap gap-2">
              {[...Array(11)].map((_, i) => (
                <div key={i} className="w-[148px] h-[78px] rounded-lg bg-gray-100 animate-pulse" />
              ))}
            </div>
            ...
            (() => {
              // Group chips by currency so each currency's accounts wrap onto
              // its own row group. Within each group the chips flow left→right;
              // 6 fit per row on xl+ at the chosen chip width (≈148px + gap).
                    ...
                    <div key={cur}>
                      {currencies.length > 1 && (
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-[10px] text-gray-400">{byCurrency[cur].length} account{byCurrency[cur].length === 1 ? '' : 's'}</span>
                        </div>
                      )}
                      <div className="flex flex-wrap gap-2">
        ...
        {selectedSubAccount && (
          <div className="border-t border-gray-200">
            ...
            <div className="overflow-x-auto relative">
              {/* Show full skeleton on first load. On view toggle (Consolidated
                  ↔ Audit), the cached data stays under a translucent overlay so
                  the user sees an obvious in-progress state. */}
              {loadingSubStatement ? (
                <div className="p-6"><TableSkeleton rows={6} /></div>
                  ...
                  {fetchingSubStatement && (
                    <div className="absolute inset-0 z-10 bg-white/60 backdrop-blur-[1px] flex items-center justify-center pointer-events-none">
                      <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white border border-gray-200 shadow-sm text-xs text-gray-600">
                        ...
                        Loading {subAccountStatementView}…
                      </div>
                    </div>
                  )}
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {(subAccountStatement.transactions || subAccountStatement.entries || []).map((txn: any, idx: number) => {
                        const dr = Number(txn.debit ?? txn.debit_amount ?? 0)
                        const cr = Number(txn.credit ?? txn.credit_amount ?? 0)
